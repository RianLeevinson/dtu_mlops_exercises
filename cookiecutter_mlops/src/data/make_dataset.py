...
def main():
    ...
    train_images = np.concatenate(
        [np.load(i, allow_pickle=True)["images"] for i in train_paths], axis=0
    )
    train_labels = np.concatenate(
        [np.load(i, allow_pickle=True)["labels"] for i in train_paths], axis=0
    )
    train = [train_images, train_labels]

    test_images = np.concatenate(
        [np.load(i, allow_pickle=True)["images"] for i in test_paths], axis=0
    )
    test_labels = np.concatenate(
        [np.load(i, allow_pickle=True)["labels"] for i in test_paths], axis=0
    )
